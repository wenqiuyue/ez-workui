  ...
  <div>
    <el-row v-if="TInfo">
      <el-col :sm="24" :md="8" class="left">
        ...
        <el-button-group>
          ...
          <el-button
            ...
            size="small"
            icon="el-icon-check"
            v-if="TInfo.btnShow[1].ishow"
            :disabled="TInfo.btnShow[1].disabled"
            @click="btnGroupClick('start')"
            >启用
          </el-button>
          ...
          <el-button
            ...
            size="small"
            icon="el-icon-close"
            v-if="TInfo.btnShow[2].ishow"
            :disabled="TInfo.btnShow[2].disabled"
            @click="btnGroupClick('close')"
            >禁用
          </el-button>
          ...
          <el-button
            ...
            size="small"
            icon="el-icon-delete"
            v-if="TInfo.btnShow[3].ishow"
            :disabled="TInfo.btnShow[3].disabled"
            @click="btnGroupClick('del')"
            >删除
          </el-button>
        </el-button-group>
      </el-col>
      ...
      <el-col :sm="24" :md="16" class="right" v-if="showSearch">
        ...
        <el-input
          v-if="TInfo.dropDown.searchInput"
          v-model="searchValue"
    ...
    <XModal
      ...
      id="pop-txt"
    >
      <div v-show="btnGroupPop[1]">
        确认要进行批量<span> [激活] </span>操作?
      </div>
      <div v-show="btnGroupPop[2]">
        确认要进行批量<span> [禁用] </span>操作?
      </div>
      <div v-show="btnGroupPop[3]">
        确认要进行批量<span> [删除] </span>操作?
      </div>
    </XModal>
  </div>
</template>
