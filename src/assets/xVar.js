...
const FileImgType = [
  {
    ...
    types:
      ",png,jpeg,jpg,jpf,jps,iff,pxr,pcx,pbm,sct,mpo,bmp,tif,tiff,gif,eps,psd,ai,webp,tga,cdr,ico,svg,fpx,exif,phd,",
  },
  {
    imgUrl: require("@/assets/fileType/audio.png"),
    types:
      ",cda,mp3,flac,wav,wma,mid,midi,rmi,xmi,ogg,ape,ai,mod,vqf,tvq,mod,aiff,au,",
  },
  {
    imgUrl: require("@/assets/fileType/video.png"),
    types:
      ",mp4,mpeg,mpg,dat,avi,navi,mov,asf,wmv,3gp,ra,rma,rmvb,mkv,flv,webm,qsv,",
  },
  {
    imgUrl: require("@/assets/fileType/zip.png"),
    types:
      ",zip,zipx,zoo,rar,7z,s7z,cab,iso,udf,bz,jar,kz,rz,wot,zz,dar,aiz,ace,ar,arc,alz,bza,fdp,gz,gzip,imp,ipk,jgz,mpkg,pak,rk,zix,",
    regex: "r\\d+|s\\d+|c\\d+|\\d+",
  },
  {
  ...
  },
  {
    ...
    types:
      ",doc,docx,docm,dot,dotm,rtf,odt,ppt,pptx,pptm,pot,potx,potm,odp,pdf,chm,pdg,wps,",
  },
  {
  ...
  install: (Vue) => {
    ...
    Vue.prototype.$loadDate = (monthNum, finalDay) => {
      ...
      for (let i = 0; i < loopMonth; i++) {
        ...
        let month = dateMonth;
        const gName = `${month.getFullYear()}-${
          parseInt(month.getMonth()) + 1
        }`;
        if (
        ...
        if (isAddDateItem) {
          ...
          //如果当月没有添加任何日期，则去除当组
          Vue.prototype.$xbb_dates = _.filter(
            Vue.prototype.$xbb_dates,
            (aa) => {
    ...
    Vue.prototype.$simplifyNum = (num, getString = false) => {
      ...
      if (getString) return result.num + " " + result.unit;
      else return result;
    };
    ...
    Vue.filter("getDividerName", function (date, arr, i) {
      ...
      const lastDay =
        i > 0 ? new Date(arr[i - 1].tsTakeTime).getYMD() : new Date();
