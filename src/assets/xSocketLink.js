	...
	constructor(opt) {
		...
		if (wsParam.token) {
			Vue.prototype.$xSocket = new xSocket({
				url: `${Vue.prototype.$socketUrl}/socket/ws.ashx?p=${JSON.stringify(wsParam)}`,
					...
					try {
						switch (command) {
							...
							case 70:
								...
								if (navigator.userAgent !== "WORK_CLIENT_AGENT_3698" && store.state.user.id !== JSON.parse(data).usId) {
						...
						if (["1", "50", "51", "53", "26", "25", "27", "70"].indexOf(res.res) >= 0) {
							opt.onMsg(res);
