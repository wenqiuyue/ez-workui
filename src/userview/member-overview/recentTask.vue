          ...
          >
            <div class="task-item-wrapper">
              <div class="task-item-left">
                <div>
                  <p @click="toTask(value, index)">
                    任务名：<span
                      :style="value.isRead ? 'color:#ccc' : ''"
                      class="name"
                      :title="value.jname"
                      >{{ value.jname }}</span
                    >
                  ...
                  <el-tag
                    type="primary"
                    size="small"
                    @click="toProject(value)"
                    >{{ value.ename }}</el-tag
                  >
                </div>
                <div>
                  在本周的执行天数：<span style="margin-right: 2rem"
                    >{{ value.exeDay }}天</span
                  >
                  实际耗时：<span>{{ formateDecimal(value.spent) }}小时</span>
                </div>
              </div>
              <div class="task-item-right">
                <div>
                  <p>
                    开始日期：{{ value.startDate.timeFormat("yyyy-MM-dd") }}
                  </p>
                  <div>
                    开始进度：
                    <el-progress
                      :text-inside="true"
                      :stroke-width="20"
                      :percentage="value.startProgress"
                    ></el-progress>
                  </div>
                </div>
                <div>
                  <p>
                    结束日期：{{
                      value.endDate
                        ? value.endDate.timeFormat("yyyy-MM-dd")
                        : value.startDate.timeFormat("yyyy-MM-dd HH:mm")
                    }}
                  </p>
                  <div>
                    结束进度：
            ...
            <div class="pro-tags" v-if="value.eveTags.length">
              ...
              <el-tag
                type="info"
                v-for="(tag, tindex) in value.eveTags"
                :key="tindex"
                :title="tag"
                >{{ tag }}</el-tag
              >
...
export default {
  ...
  methods: {
    ...
    getTaskActiveList() {
      this.isLoading = true;
      this.$http
        .get("/Company/MemberJob/RecentJobs.ashx", {
          params: {
            u: this.activeId,
