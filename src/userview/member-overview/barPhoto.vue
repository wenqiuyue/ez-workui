...
<template>
	<div>
		<XModal width="60%" height="80%" :name="customName" :showCrossBtn="true" :title="activeBar.name+'类的截图'" class="work-timeAxis"
		 @beforeOpen="dataInit" @opened="open()">
			<div style="overflow: auto;height: 100%;" :infinite-scroll-disabled="photoDis" v-infinite-scroll="loadMoreShot"
		...
		methods: {
			...
			getBarData() {
				...
				let params = {
					...
					uid: this.activeBar.id,
					stime: this.st,
					etime: this.et,
					last: this.last,
					pageCount: 15
				}
				if(this.activeBar.date){
					delete params.etime
					params.stime = this.activeBar.date
				}
