    ...
    >
      ...
      <div class="people_list" v-if="memberData && memberData.length">
        <div class="list_card" v-for="(item, index) in memberData" :key="index">
          ...
          <!-- <div class="work_time">
            <p>
              <span>本{{ listdateType == 1 ? "周" : "月" }}工作时间：</span>
              <span>{{ item.WorkTime }}小时</span>
            </p>
            <p>
              <span>本{{ listdateType == 1 ? "周" : "月" }}加班时间：</span>
              <span>{{ item.AddWorkTime }}小时</span>
            </p>
            <p>
              <span>本{{ listdateType == 1 ? "周" : "月" }}行为分析：</span>
              <span>{{
                item.BehaviorPrediction ? item.BehaviorPrediction : "无"
              }}</span>
            </p>
          </div> -->
