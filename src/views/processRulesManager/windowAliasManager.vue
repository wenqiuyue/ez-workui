...
export default {
  ...
  methods: {
    ...
    handleCancle(row) {
      this.$confirm("此操作将删除该别名, 是否继续?", "提示", {
        ...
        .then(() => {
          this.$http
            .post(
              "/Management/ProgressManagement/DelSystemProgressAlias.ashx",
              {
                Ids: [row.Id],
              }
            )
    ...
    editConfirm() {
      ...
      this.$http
        .post(
          "/Management/ProgressManagement/EditSystemProgressAlias.ashx",
          data
        )
        .then((resp) => {
          if (resp.res == 0) {
    ...
    addConfirm() {
      ...
      this.$refs.xmodal.loadBtn(true);
      this.$http
        .post("/Management/ProgressManagement/AddSystemProgressAlias.ashx", {
          name: this.roleForm.name,
          formnames: fm.map((m) => m.str),
          configId: this.selRow.Id,
        })
    ...
    getNameData() {
      ...
      this.$http
        .get("/Management/ProgressManagement/GetSystemProgressAliasList.ashx", {
          params: data,
        })
        .then((resp) => {
          if (resp.res == 0) {
