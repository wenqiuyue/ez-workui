<template>
  <XModel
    isFixed
    ref="xmodal"
    :name="isChildTask ? 'addTaskChild' : customName"
    :title="isChildTask ? '创建子任务' : '创建任务'"
    cancelBtnText="取消"
    confirmBtnText="确定"
    ...
    <div class="t-wrapper">
      <div class="t-form">
        <el-form ref="taskForms" class="form" :model="taskForm" :rules="rules">
          <el-form-item class="form_item" prop="name" label="任务名称：">
            <el-input
          ...
          <el-form-item
            class="form_item"
            label="所属需求："
            v-if="taskForm.eventeID && !isChildTask && !isDefaultDemand"
          >
            <SelDemand
              @demandConfirm="handleSelDemand"
...
export default {
  components: {
    ...
    SelDemand: () => import("@/components/Selectors/selDemand"),
  },
  props: {
    //执行人
    frontman: {
  ...
  methods: {
    ...
    taskLoopOk() {
      if (this.postType == 1) {
        const data = {
          Condition: this.looptype,
          LoopType: this.loopday,
          LoopTime1: "",
          LoopTime2: this.value4,
        };
        this.taskForm.loop = data;
      } else if (this.postType == 2) {
        const data = {
          type: this.looptype,
          LoopType: this.loopmonth,
        ...
        const data = {
          type: this.looptype,
          LoopType: this.loopweek2,
          LoopTime1: this.loopweek,
          LoopTime2: this.value4,
        };
        this.taskForm.loop = data;
      ...
      this.$notify.success("设置完成");
      this.$modal.hide("taskLoopNew2");
    },
    typeone(val) {
      this.postType = val;
    ...
    handleSelDemand(dem) {
      if (dem.demandM) {
        this.taskForm.demandId = dem.demandM.Id;
      }
      if (dem.demandR) {
        this.taskForm.demandFlowStep = dem.demandR.Step ? dem.demandR.Step : 0;
      }
    },
    close() {
    ...
    beforeOpen() {
      ...
      if (this.isHome) {
        ...
        this.getLists(this.taskForm.eventeID);
        this.$nextTick(() => {
          this.taskForm.demandId = this.demand;
          this.taskForm.demandFlowStep = this.verifySetp ? this.verifySetp : 0;
        });
      }
    },
    searchSimilar() {
